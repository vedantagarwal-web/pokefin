...
async def generate_chart(
    ...
    chart_type: price, volume, candlestick
    """
    if chart_type in ["price", "candlestick"]:
        prices = await fd_client.get_historical_prices(ticker, timeframe)
        ...
        return {
            ...
            "data": prices,
            "config": {
                "type": "line" if chart_type == "price" else "candlestick",
                "x_axis": "date",
                "y_axis": "close" if chart_type == "price" else ["open", "high", "low", "close"]
            }
        }
